...
class Container implements UIComponent {
  ...
  getWidth(): number {
    ...
    // 세로 배치면 가장 넓은 것
    return Math.max(...this.children.map((c) => c.getWidth())) + this.padding * 2;
  }
  ...
  getHeight(): number {
    ...
    return Math.max(...this.children.map((c) => c.getHeight())) + this.padding * 2;
  }
}
```
...
const cardList = new Container("row", 8)
  .add(card)
  .add(card)
  .add(card);
